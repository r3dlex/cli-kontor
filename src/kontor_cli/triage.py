...
class Triage:
    ...
    def __init__(self, config: Config, cwd: Path | None = None) -> None:
        ...
        # Deterministic folder classifier, built lazily — gives each candidate
        # its canonical taxonomy placement as context (rules-only, no LLM). Kept
        # out of __init__ so the create_task_for path never loads rule files.
    ...
    def _canonical_folder(self, email: Email) -> str | None:
        """Best-effort canonical taxonomy folder via the deterministic rules
        engine (lazily built). Returns None if rules don't match or are
        unavailable — it is advisory context, never a hard dependency."""
        if not self._rules_init:
            self._rules_init = True
            try:
                self._rules = RulesEngine(self.config, self.cwd)
            except Exception as exc:  # noqa: BLE001 — canonical hint is optional
                logger.debug("rules engine unavailable for triage context: %s", exc)
                self._rules = None
        if self._rules is None:
            return None
        try:
            return self._rules.classify(email)
